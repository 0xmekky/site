...
function accumulate_bitstring(label, ary, spacing) {
  ...
  for (i = 1; i < ary.length; i++) {
    if (i % spacing == 1) accumulated_output_info += " "; // time to add a space
...
let asciiBtn = document.getElementById("asciiBtn");
...
asciiBtn.addEventListener("click", function () {
  ...
  let asciiValue = stringToASCII(inData);
  document.getElementById("EncValue").value = asciiValue.join("");
});

function stringToASCII(inData) {
  return Array.from(inData).map((char) => char.charCodeAt(0));
}

hexBtn.addEventListener("click", function () {
  let inData = document.getElementById("indata").value;
  let hexValue = textToHex(inData);
  document.getElementById("EncValue").value = hexValue;
});
